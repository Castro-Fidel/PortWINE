...
portwine_start_debug () {
    ...
    echo "$(cat /proc/cpuinfo | grep 'cpu cores' | sort -u | cut -d':' -f2)-physical $(cat /proc/cpuinfo | \
    grep 'processor' | wc -l)-logical cores $(cat /proc/cpuinfo | grep 'model name' | sort -u | cut -d':' -f2)" >> "${PORT_WINE_PATH}/${portname}.log"
    echo "------------------------------------------------" >> "${PORT_WINE_PATH}/${portname}.log"
